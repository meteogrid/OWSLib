...
class WebCoverageService_2_0_1(WCSBase):
    ...
    def getCoverage(self, identifier=None, format = None, subsets=None, method='Get', **kwargs):
...
class ContentMetadata(object):
    ...
    def __init__(self, elem, parent, service):
        ...
        # bboxes - other CRS 
        self.boundingboxes = []
        for bbox in elem.findall('{http://www.opengis.net/ows}boundedBy'):
            for b in bbox.findall('{http://www.opengis.net/ows}EnvelopeWithTimePeriod'):
                lc = [float(c) for c in b.find('{http://www.opengis.net/ows}LowerCorner').text.split()]
                uc = [float(c) for c in b.find('{http://www.opengis.net/ows}UpperCorner').text.split()]
                lc += [iso8601.parse_date(b.find('{http://www.opengis.net/ows}beginPosition').text)]
                uc += [iso8601.parse_date(b.find('{http://www.opengis.net/ows}endPosition').text)]
                boundingBox = (lc,uc)
        ...
        #SupportedCRS
        self.supportedCRS=[]
        for crs in elem.findall('{http://www.opengis.net/wcs/2.0}SupportedCRS'):
            self.supportedCRS.append(Crs(crs.text))
'''            
            
        #SupportedFormats         
        self.supportedFormats=[]
        for format in elem.findall('{http://www.opengis.net/wcs/2.0}SupportedFormat'):
